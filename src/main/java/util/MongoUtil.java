...
public enum MongoUtil {
    ...
    static {
        ...
        // mongo.mongoClient = new MongoClient(ip, port);
        // 大部分用户使用mongodb都在安全内网下，但如果将mongodb设为安全验证模式，就需要在客户端提供用户名和密码：
        try {
            mongoClient = MongoConectionUtil.getMongoClient(ip, port, userName, passWord);
        ...
        options.connectTimeout(3000000);// 连接超时，推荐>3000毫秒
        options.maxWaitTime(500000); //
    ...
    public static void main(String[] args) {
        ...
        MongoCollection<Document> coll = MongoUtil.mongo.getCollection(dbName, collName);
